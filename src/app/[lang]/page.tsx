...
export async function generateMetadata({ params }: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: "Plastic Love — Trusted Plastic Surgery Insights from Seoul",
      description: "Evidence-based plastic surgery information from a board-certified surgeon in Seoul.",
...
export default async function HomePage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const grid = posts.slice(3, 7);
  const numbered = posts.slice(0, 4);
      ...
      <section className="mb-16 bg-card rounded-2xl shadow-sm p-8 md:p-10 flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="text-center md:text-left">
          ...
          <p className="text-sub text-sm mb-4 leading-relaxed max-w-lg">
            Board-certified plastic surgeon specializing in anti-aging surgery — facelift and upper/lower blepharoplasty — at VIP Plastic Surgery. Sharing evidence-based insights to help you make informed decisions.
          </p>
